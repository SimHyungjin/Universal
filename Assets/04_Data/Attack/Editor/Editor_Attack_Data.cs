...
public class Editor_Attack_Data : Editor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        do
        {
            if (prop.propertyPath == "additionalHits")
            {
                ...
                if (targets.Length > 1)
                    EditorGUILayout.HelpBox("Additional Hits editing is only available when one attack asset is selected.", MessageType.Info);
                else
                    _list.DoLayoutList();
                continue;
            }
    ...
    {
        SO_Attack_Data so = (SO_Attack_Data)target;
