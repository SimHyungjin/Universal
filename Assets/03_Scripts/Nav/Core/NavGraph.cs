...
namespace MapNav.Core
{
    ...
    public static class NavGraph
    {
            ...
            out float2 neighborArrival)
        {
            ...
            {
                if (!TrySelectTransitionToRegionAccess(in ctx, ref blob, edge.ToId, fromLocal, edge, agentRadius, out float2 endpoint, out float2 regionPoint))
                {
                    cost = 0f;
                    return false;
                }
        ...
        private static void TryConsiderTransitionAccess(
            in NavContext ctx,
            ref NavBlob blob,
            int regionId,
            float2 fromLocal,
            float2 candidateEndpoint,
            float agentRadius,
            ref bool found,
            ref float bestCost,
            ref float2 bestEndpoint,
            ref float2 bestRegionPoint)
        {
            if (!TryGetRegionAccessPointLocal(ref blob, regionId, candidateEndpoint, out float2 candidateRegionPoint))
                return;

            if (!NavInRegion.TryMeasurePathLocal(in ctx, regionId, candidateRegionPoint, candidateRegionPoint, agentRadius, out _))
                return;

            float cost = math.distance(fromLocal, candidateEndpoint) + math.distance(candidateEndpoint, candidateRegionPoint);
            ...
            bestRegionPoint = candidateRegionPoint;
        }
